...
## 3.1. Definition.
The definition of categories looks complicated at first, but can be summarized as a collection of 'objects' and of 'morphisms' betweem these objects, satisfying some natural conditions.  
...
- The identity morhpisms are identities with respect to composition: That is, for all $f\in\mathrm{Hom}_\mathcal{C}(A,B)$ we have $$f1_A=f,\quad1_Bf=f.$$
...
A morhpism of an object $A$ of a category $C$ to itself is called an *endomorphism*; $\mathrm{Hom}_\mathcal{C}(A,A)$ is denoted $\mathrm{End}_\mathcal{C}(A)$. 
...
Suppose $S$ is a set and $\sim$ is a relation on $S$ satisfying the rflexive and transitive properties. Then we can encode this data into a category:
...
Let $\mathrm{C}$ be a cateogry, and let $A$ be an object of $\mathrm{C}$. We are going to define a category $\mathrm{C}_A$ whose objects are certain *morhpisms* in $\mathrm{C}$, and whose morphsms are certain *diagrams* of $\mathrm{C}$. 
- $\mathrm{Obj}(\mathrm{C}_A)=$ all morhpisms from any object of $\mathrm{C}$ to $A$; thus, an object of $\mathrm{C}_A$ is a morphism $f\in\mathrm{Hom}_\mathrm{C}(Z,A)$ for some object $Z$ of $C$. Pictorially, an object of $\mathrm{C}_A$ is an arrow $Z\xrightarrow{f}A$ in $\mathrm{C}$; these are often drop 'top-down'. 
...
A morhpism between two such objects, $(S,s)\rightarrow(T,t)$, corresponds then to a set-function $\sigma:S\rightarrow T$ *such that* $\sigma(s)=t$  
